...
temp = read_csv("Raw/FracFocus_NM_TX_PermianBasin_2009_2019_new.csv", 
                guess_max=10000, col_types = c(County = "c", Year = "i")) %>% #added _new for Year/month and HUC12
  rename(Volume = TotalBaseWaterVolume,
         Depth = TotalVerticalDepth) %>%
  mutate(Count = 1) %>% 
  mutate(County = as.character(County)) #converting County column from double to character values
dat = temp %>%
